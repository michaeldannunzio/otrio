...
/*
 * `isMissingRelay` used to live here as a regex over the transport's prose.
 * It now lives in `src/net/rtcTransport.ts`, next to the constant it tests, and
 * is imported from there.
 *
 * The reason is worth keeping: a string copied across a module boundary is a
 * divergence with a delay fuse. Reword the message and the predicate keeps
 * compiling, keeps passing, and quietly stops matching — so the retry button
 * comes back in the one branch where retrying provably cannot work.
 */
